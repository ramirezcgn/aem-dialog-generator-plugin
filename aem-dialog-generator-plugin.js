...
class AemDialogGeneratorPlugin {
    constructor(options = {}) {
        ...
        this.options = {
            ...
            targetDir: options.targetDir ||
                path.resolve(__dirname, '../ui.apps/src/main/content/jcr_root/apps/mysite/components'),
            ...
            useFolderStructure: options.useFolderStructure !== undefined ? options.useFolderStructure : true,
    ...
    generateDialogs() {
        ...
        componentFolders.forEach(componentName => {
            const dialogJsonPath = path.join(sourceDir, componentName, dialogFileName);
            
            if (fs.existsSync(dialogJsonPath)) {
                this.log(`Processing: ${componentName}`);
                
                try {
                    const dialogConfig = JSON.parse(fs.readFileSync(dialogJsonPath, 'utf8'));
                    const xmlContent = this.generateDialogXml(dialogConfig, componentName);
    ...
    generateDialogXml(config, componentName) {
        ...
        xml += this.line(0, '<jcr:root xmlns:sling="http://sling.apache.org/jcr/sling/1.0" xmlns:jcr="http://www.jcp.org/jcr/1.0"');
        xml += this.line(2.5, 'xmlns:nt="http://www.jcp.org/jcr/nt/1.0" xmlns:cq="http://www.day.com/jcr/cq/1.0"');
        xml += this.line(2.5, 'xmlns:granite="http://www.adobe.com/jcr/granite/1.0"');
        xml += this.line(1, 'jcr:primaryType="nt:unstructured"');
        xml += this.line(1, `jcr:title="${this.escapeXml(title)}"`);
        xml += this.line(1, 'sling:resourceType="cq/gui/components/authoring/dialog">');
        ...
        const useSimpleLayout = layout === 'simple' || (tabs.length === 0 && (fields.length > 0 || items.length > 0));
        ...
        if (useSimpleLayout) {
            ...
            xml += this.line(4, 'sling:resourceType="granite/ui/components/coral/foundation/fixedcolumns"');
            ...
            xml += this.line(6, 'sling:resourceType="granite/ui/components/coral/foundation/container">');
            ...
            xml += this.line(2, '<items jcr:primaryType="nt:unstructured">');
            xml += this.line(3, '<tabs jcr:primaryType="nt:unstructured">');
            xml += this.line(4, '<items jcr:primaryType="nt:unstructured">');
    ...
    generateTab(tab, tabIndex) {
        ...
        const tabName = tab.name || this.sanitizeNodeName(title).toLowerCase() || `tab${tabIndex}`;
        ...
        xml += this.line(8, 'sling:resourceType="granite/ui/components/coral/foundation/fixedcolumns"');
        ...
        xml += this.line(10, 'sling:resourceType="granite/ui/components/coral/foundation/container">');
    ...
    generateField(field) {
        ...
        if (description) {
            xml += this.line(this.I.FA, `fieldDescription="${this.escapeXml(description)}"`);
        }
        ...
        if (defaultValue !== undefined) {
            xml += this.line(this.I.FA, `value="${this.escapeXml(defaultValue.toString())}"`);
        }
        ...
        if (options && Array.isArray(options) && options.length > 0) {
            ...
            options.forEach((option, index) => {
                ...
                xml += this.line(this.I.FNI + 1, `text="${this.escapeXml(option.text || option.value)}"`);
                xml += this.line(this.I.FNI + 1, `value="${this.escapeXml(option.value)}"/>`);
            });
    ...
    generateFieldset(field) {
        ...
        const nodeName = name ? this.sanitizeNodeName(name) : `fieldset_${Date.now()}`;
        ...
        fieldsetFields.forEach(subField => {
    ...
    generateRTE(field) {
        ...
        // RTE configuration
        xml += this.line(this.I.FN, '<rtePlugins jcr:primaryType="nt:unstructured">');
        ...
        // UI Config
        xml += this.line(this.I.FN, '<uiSettings jcr:primaryType="nt:unstructured">');
        ...
        xml += this.line(this.I.FNI + 2, 'toolbar="[format#bold,format#italic,format#underline,#justify,#lists,links#modifylink,links#unlink,fullscreen#start]"');
        xml += this.line(this.I.FNI + 2, 'popovers="[justify#justifleft,justify#justifycenter,justify#justifyright,lists#unordered,lists#ordered,links#link]">');
        xml += this.line(this.I.FNI + 2, '<icons jcr:primaryType="nt:unstructured">');
        xml += this.line(this.I.FNI + 3, '<justify jcr:primaryType="nt:unstructured">');
        xml += this.line(this.I.FNI + 4, '<justifyleft command="justifyleft"/>');
        xml += this.line(this.I.FNI + 4, '<justifycenter command="justifycenter"/>');
        xml += this.line(this.I.FNI + 4, '<justifyright command="justifyright"/>');
        xml += this.line(this.I.FNI + 3, '</justify>');
        xml += this.line(this.I.FNI + 3, '<lists jcr:primaryType="nt:unstructured">');
    ...
    generateRTEDefaultPlugins() {
        ...
        xml += this.line(this.I.FNI + 1, 'jcr:primaryType="nt:unstructured"');
        xml += this.line(this.I.FNI + 1, 'features="*">');
        xml += this.line(this.I.FNI + 1, '<formats jcr:primaryType="nt:unstructured">');
    ...
    generateRTEPlugin(feature) {
        const plugins = {
            ...
            'underline': '<format jcr:primaryType="nt:unstructured" features="underline"/>',
            'links': '<links jcr:primaryType="nt:unstructured" features="modifylink,unlink"/>',
    ...
    generateMultifield(field) {
        ...
        if (composite && fields.length > 0) {
            ...
            xml += this.line(this.I.FNI, 'sling:resourceType="granite/ui/components/coral/foundation/container"');
            ...
            const subResourceType = this.getResourceType(singleField.type || 'textfield');
            
            xml += this.line(this.I.FNI, `sling:resourceType="${subResourceType}"`);
            xml += this.line(this.I.FNI, `name="${fieldName}"`);
            
            if (singleField.label) {
                xml += this.line(this.I.FNI, `fieldLabel="${this.escapeXml(singleField.label)}"`);
            }
            
            if (singleField.description) {
                xml += this.line(this.I.FNI, `fieldDescription="${this.escapeXml(singleField.description)}"`);
            }

            // Add additional properties of internal field
            for (const [key, value] of Object.entries(singleField)) {
                if (key !== 'type' && key !== 'label' && key !== 'description' && key !== 'name') {
                    const attr = this.generateAttributeValue(key, value);
            ...
            xml += this.line(this.I.FNI, 'sling:resourceType="granite/ui/components/coral/foundation/form/textfield"');
            xml += this.line(this.I.FNI, `name="${fieldName}"/>`);
        }
    ...
    generateMultifieldItem(field) {
        ...
        if (description) {
            xml += this.line(this.I.MI + 1, `fieldDescription="${this.escapeXml(description)}"`);
        }
        ...
        if (defaultValue !== undefined) {
            xml += this.line(this.I.MI + 1, `value="${this.escapeXml(defaultValue.toString())}"`);
        }

        Object.entries(otherProps).forEach(([key, value]) => {
            if (key !== 'options') {
                xml += this.line(this.I.MI + 1, this.generateAttributeValue(key, value));
            }
        });

        if (options && Array.isArray(options) && options.length > 0) {
            xml = xml.trimEnd() + '>\n';
            xml += this.line(this.I.MI + 1, '<items jcr:primaryType="nt:unstructured">');
            ...
            options.forEach((opt, i) => {
                ...
                xml += this.line(this.I.MI + 3, `text="${this.escapeXml(opt.text || opt.value)}"`);
                xml += this.line(this.I.MI + 3, `value="${this.escapeXml(opt.value)}"/>`);
            });
